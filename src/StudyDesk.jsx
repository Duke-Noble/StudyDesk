...
const G = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .clk:hover{filter:brightness(.96);transform:translateY(-1px);}
...
function useTable(table, orderCol="created_at") {
  ...
  const load = useCallback(async () => {
    ...
    if (error) { setDbError(error.message); setRows([]); }
    else { setRows(data||[]); }
    setLoading(false);
  },[table,orderCol]);
  useEffect(()=>{ load(); },[load]);
...
function OD() {
  return <span style={{fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:99,background:BRAND.dangerBg,color:BRAND.danger,border:"1px solid #F5C0BC"}}>OVERDUE</span>;
}
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      <input type={type} value={value??""} onChange={onChange} placeholder={placeholder} min={min} max={max}
        ...
        style={{width:"100%",padding:"10px 13px",borderRadius:9,border:`1.5px solid ${f?BRAND.navyMid:BRAND.line}`,fontSize:13,color:BRAND.navy,background:BRAND.white,outline:"none",fontFamily:"inherit",transition:"border .15s",...style}}/>
    </div>
  );
}
function Sel({ label, value, onChange, options, required }) {
  return (
    <div style={{marginBottom:14}}>
      {label&&<label style={{fontSize:11,fontWeight:700,color:BRAND.slate,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:".06em"}}>{label}{required&&<span style={{color:BRAND.danger}}> *</span>}</label>}
      <select value={value??""} onChange={onChange} style={{width:"100%",padding:"10px 13px",borderRadius:9,border:`1.5px solid ${BRAND.line}`,fontSize:13,color:BRAND.navy,background:BRAND.white,outline:"none",fontFamily:"inherit",cursor:"pointer"}}>
...
function TA({ label, value, onChange, placeholder, rows=3 }) {
  return (
    <div style={{marginBottom:14}}>
      {label&&<label style={{fontSize:11,fontWeight:700,color:BRAND.slate,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:".06em"}}>{label}</label>}
      <textarea value={value??""} onChange={onChange} placeholder={placeholder} rows={rows}
        style={{width:"100%",padding:"10px 13px",borderRadius:9,border:`1.5px solid ${BRAND.line}`,fontSize:13,color:BRAND.navy,background:BRAND.white,outline:"none",fontFamily:"inherit",resize:"vertical"}}/>
    </div>
  );
}
function ErrBox({ msg }) {
  return msg?<div style={{padding:"12px 15px",borderRadius:10,background:BRAND.dangerBg,border:"1px solid #F5C0BC",color:BRAND.danger,fontSize:13,marginBottom:15,lineHeight:1.5}}>❌ {msg}</div>:null;
}
function Modal({ title, onClose, children, wide=false }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(8,18,32,.50)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16,backdropFilter:"blur(5px)"}}
      onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div className="fi" style={{background:BRAND.white,borderRadius:18,width:wide?"min(720px,96vw)":"min(540px,96vw)",maxHeight:"92vh",overflowY:"auto",boxShadow:"0 32px 80px rgba(8,18,32,.25)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"22px 26px 16px",borderBottom:`1px solid ${BRAND.line}`}}>
...
function Empty({ icon="note", title, sub, action, onAction }) {
  return (
    <div style={{textAlign:"center",padding:"40px 24px",color:BRAND.muted}}>
      <div style={{width:56,height:56,borderRadius:16,background:BRAND.navyLt,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",color:BRAND.navy}}><Ic n={icon} size={26}/></div>
      <div style={{fontSize:14,fontWeight:700,color:BRAND.slate,marginBottom:6}}>{title}</div>
      {sub&&<div style={{fontSize:13,marginBottom:16,color:BRAND.muted}}>{sub}</div>}
...
function ColorPicker({ value, onChange }) {
  return (
    <div style={{marginBottom:16}}>
      <label style={{fontSize:11,fontWeight:700,color:BRAND.slate,display:"block",marginBottom:10,textTransform:"uppercase",letterSpacing:".06em"}}>Colour Tag</label>
      <div style={{display:"flex",gap:12}}>
        {COLORS.map(c=>{const col=PAL[c];return(
          <button key={c} type="button" onClick={()=>onChange(c)} title={c}
            style={{width:36,height:36,borderRadius:"50%",background:col.accent,border:value===c?`3px solid ${BRAND.navy}`:"3px solid transparent",cursor:"pointer",boxShadow:value===c?`0 0 0 2px #fff inset,0 0 0 4px ${col.accent}`:"none",transition:"all .15s"}}/>
        );})}
      </div>
    </div>
  );
...
function RichEd({ value, onChange, placeholder="Write your notes here..." }) {
  ...
  const tools=[
    ...
    {l:"1.",c:()=>cmd("insertOrderedList"),s:{fontSize:11}},{l:"—",c:()=>{cmd("removeFormat");cmd("formatBlock","p");},s:{color:"#94a3b8"}},
  ];
  return (
    <div style={{border:`1.5px solid ${BRAND.line}`,borderRadius:10,overflow:"hidden"}}>
      <div style={{display:"flex",gap:2,padding:"6px 8px",background:BRAND.bg,borderBottom:`1px solid ${BRAND.line}`,flexWrap:"wrap"}}>
        {tools.map(t=><button key={t.l} onMouseDown={e=>{e.preventDefault();t.c();}} style={{...t.s,minWidth:28,height:28,border:`1px solid ${BRAND.line}`,borderRadius:6,cursor:"pointer",background:BRAND.white,color:BRAND.navy,padding:"0 4px"}}>{t.l}</button>)}
      </div>
...
function Skeleton() {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14,padding:"28px 0"}}>
      {[1,2,3].map(i=><div key={i} style={{height:76,borderRadius:13,background:"linear-gradient(90deg,#E8EEF5 25%,#D8E2EC 50%,#E8EEF5 75%)",backgroundSize:"400% 100%",animation:"shim 1.5s infinite"}}/>)}
    </div>
  );
}
...
function NoteForm({ form, setForm, save, saving, onCancel, err, showCourseSelect, courses }) {
  ...
  return (
    <>
      <ErrBox msg={err}/>
      <div style={{marginBottom:18,paddingBottom:16,borderBottom:`1px solid ${BRAND.line}`}}><BtnRow/></div>
      ...
      <div style={{marginBottom:14}}>
        <label style={{fontSize:11,fontWeight:700,color:BRAND.slate,display:"block",marginBottom:8,textTransform:"uppercase",letterSpacing:".06em"}}>Content</label>
        <RichEd value={form.content||""} onChange={v=>setForm(p=>({...p,content:v}))}/>
      </div>
      <div style={{paddingTop:16,borderTop:`1px solid ${BRAND.line}`}}><BtnRow/></div>
    </>
  );
}
...
function Dashboard({ courses, deadlines, quizzes, assessments, assignments, notes, slots, user, onNav, onSelectCourse, dbError }) {
  ...
  const upcoming = [
    ...
    ...quizzes.filter(x=>x.status!=="Completed"&&isSoon(x.date_time)).map(x=>({...x,_t:"Quiz",_d:x.date_time,_c:"#5B2D9E",_nav:"calendar"})),
  ...
  const overdue = [
    ...
    ...assessments.filter(x=>x.status!=="Graded"&&x.status!=="Submitted"&&isOD(x.due_date)).map(x=>({...x,_t:"Assessment",_d:x.due_date,_c:"#1A6DBF",_nav:"calendar"})),
  ].sort((a,b)=>new Date(a._d)-new Date(b._d));
  ...
  /*return (
    <div className="fi">
      ...
      <div style={{marginBottom:24,padding:"24px 32px",borderRadius:20,color:"#fff",background:"linear-gradient(135deg,#0F2D52 0%,#1A4173 60%,#0E3A6E 100%)"}}>
        ...
        <h1 style={{fontSize:26,fontWeight:800,lineHeight:1.25,fontFamily:"'Lora',serif"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:24}}>
        {[
          ...
          <div key={card.label} className="clk hl" onClick={()=>onNav(card.nav)}
            style={{background:BRAND.white,borderRadius:16,padding:"20px 18px",border:`1px solid ${BRAND.line}`,display:"flex",alignItems:"flex-start",gap:14}}>
            <div style={{width:48,height:48,borderRadius:13,background:card.accent+"1A",display:"flex",alignItems:"center",justifyContent:"center",color:card.accent,flexShrink:0}}>
              <Ic n={card.icon} size={22}/>
            </div>
            <div>
              <div style={{fontSize:30,fontWeight:800,color:BRAND.navy,lineHeight:1}}>{card.value}</div>
              <div style={{fontSize:12,color:BRAND.muted,marginTop:5,fontWeight:500}}>{card.label}</div>
              {card.sub&&<div style={{fontSize:11,color:card.accent,fontWeight:700,marginTop:3}}>{card.sub}</div>}
            </div>
          </div>
        ))}
      </div>
                ...
                return(
                  <div key={item.id} className="clk" onClick={()=>onNav("calendar")}
                    ...
                    <div style={{display:"flex",alignItems:"flex-start",gap:6}}>
                      <div style={{width:3,borderRadius:2,alignSelf:"stretch",background:item._c,minHeight:28,flexShrink:0}}/>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontWeight:700,fontSize:12,color:BRAND.navy,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:3}}>{item.title}</div>
              ...
              {overdue.slice(0,6).map(item=>{
                ...
                return(
                  <div key={item.id} className="clk" onClick={()=>onNav("calendar")}
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:700,fontSize:12,color:BRAND.navy,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:2}}>{item.title}</div>
      ...
      {courses.length>0&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <h2 style={{fontSize:14,fontWeight:800,color:BRAND.navy}}>📚 My Courses</h2>
...
function CoursesPage({ courses, reload, onSelect, dbError }) {
  ...
  return(
    <div className="fi">
      ...
      {courses.length===0?(
        <Empty icon="book" title="No courses yet" sub="Click '+ Add Course' to add your first course. Up to 8 allowed." action="Add Course" onAction={()=>{setForm({color_tag:"Blue"});setModal("add");}}/>
      ):(
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:18}}>
          {courses.map(c=>{const col=PAL[c.color_tag]||PAL.Blue;return(
            <div key={c.id} className="hl" style={{background:BRAND.white,borderRadius:16,border:`1.5px solid ${col.border}`,overflow:"hidden"}}>
              ...
              <div style={{padding:"18px 20px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div style={{flex:1,cursor:"pointer"}} onClick={()=>onSelect(c)}>
                    ...
                    <div style={{fontSize:15,fontWeight:800,color:BRAND.navy,lineHeight:1.4,marginBottom:10}}>{c.course_name}</div>
                    <div style={{display:"flex",flexDirection:"column",gap:5}}>
                      ...
                      {!c.instructor_name&&!c.schedule&&!c.room&&<div style={{fontSize:12,color:"#A0AEC0",fontStyle:"italic"}}>Tap Edit to add details</div>}
                    </div>
                  </div>
...
function CourseDetail({ course, notes, deadlines, quizzes, assessments, assignments, reloadAll, onBack }) {
  ...
  const Row=({item,type,meta})=>{
    ...
    return(
      <div style={{display:"flex",gap:12,padding:"12px 15px",borderRadius:11,marginBottom:9,background:od?BRAND.dangerBg:BRAND.bg,border:`1px solid ${od?"#F5C0BC":BRAND.line}`,alignItems:"center"}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontWeight:700,fontSize:14,color:BRAND.navy,marginBottom:5,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>{item.title}{od&&<OD/>}</div>
          <div style={{fontSize:11,color:BRAND.muted,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>{meta}</div>
        </div>
        <div style={{display:"flex",gap:6}}>
  ...
  return(
    <div className="fi">
      ...
      <div style={{background:BRAND.white,borderRadius:18,border:`1.5px solid ${col.border}`,overflow:"hidden",marginBottom:24}}>
        ...
        <div style={{padding:"22px 26px"}}>
          ...
          <h1 style={{fontSize:20,fontWeight:800,color:BRAND.navy,marginTop:12,marginBottom:16,fontFamily:"'Lora',serif"}}>{course.course_name}</h1>
          <div style={{display:"flex",gap:26,flexWrap:"wrap"}}>
            {[["👤 Instructor",course.instructor_name||"—"],["📍 Room",course.room||"—"],["🕐 Schedule",course.schedule||"—"],["📧 Email",course.instructor_email||"—"]].map(([l,v])=>(
              <div key={l}><div style={{fontSize:10,color:"#A0AEC0",fontWeight:700,textTransform:"uppercase",letterSpacing:".05em"}}>{l}</div><div style={{fontSize:13,color:BRAND.slate,fontWeight:600,marginTop:2}}>{v}</div></div>
            ))}
          </div>
        </div>
      </div>
      ...
      {tab==="notes"&&(
        <div>
          ...
          {cN.length===0?<Empty icon="note" title="No notes yet" sub="Add your first note" action="Add Note" onAction={()=>open("note",null)}/>:
            cN.map(note=>(
              <div key={note.id} style={{background:BRAND.white,borderRadius:13,border:`1px solid ${BRAND.line}`,marginBottom:13,overflow:"hidden"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",background:BRAND.bg,borderBottom:`1px solid ${BRAND.line}`}}>
                  <div><div style={{fontWeight:800,fontSize:15,color:BRAND.navy,fontFamily:"'Lora',serif"}}>{note.title}</div><div style={{fontSize:11,color:BRAND.muted,marginTop:2}}>Updated {fmtDate(note.updated_at)}</div></div>
                  <div style={{display:"flex",gap:6}}>
                ...
                <div style={{padding:"16px 18px",minHeight:48}}>
                  {note.content?<div style={{fontSize:14,color:BRAND.slate,lineHeight:1.8}} dangerouslySetInnerHTML={{__html:note.content}}/>:<div style={{fontSize:13,color:BRAND.muted,fontStyle:"italic"}}>No content yet — tap Edit to write your notes.</div>}
                </div>
              </div>
            ))
          }
  ...
  return(
    <div className="fi">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
        <div><h1 style={{fontSize:24,fontWeight:800,color:BRAND.navy}}>Notes</h1><p style={{fontSize:13,color:BRAND.muted,marginTop:3}}>All lecture notes across your courses</p></div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
        <div style={{position:"relative",flex:1,minWidth:200}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search notes..." style={{width:"100%",padding:"10px 13px 10px 36px",borderRadius:9,border:`1.5px solid ${BRAND.line}`,fontSize:13,background:BRAND.white,outline:"none",boxSizing:"border-box",color:BRAND.navy}}/>
        </div>
        <select value={cf} onChange={e=>setCf(e.target.value)} style={{padding:"10px 14px",borderRadius:9,border:`1.5px solid ${BRAND.line}`,fontSize:13,background:BRAND.white,outline:"none",minWidth:180,color:BRAND.navy}}>
          <option value="">All Courses</option>
          {courses.map(c=><option key={c.id} value={c.id}>{c.course_code}</option>)}
        </select>
      </div>
      {filtered.length===0?<Empty icon="note" title="No notes found" sub={search?"Try a different term":"Add your first note"} action="New Note" onAction={()=>{setModal({mode:"add"});setForm({});}}/>:
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:15}}>
          {filtered.map(note=>{const c=gc(note.course_id);const col=c?PAL[c.color_tag]:PAL.Blue;return(
            <div key={note.id} style={{background:BRAND.white,borderRadius:14,border:`1px solid ${BRAND.line}`,overflow:"hidden"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"14px 16px",background:BRAND.bg,borderBottom:`1px solid ${BRAND.line}`,position:"relative"}}>
                <div style={{position:"absolute",top:0,left:0,bottom:0,width:4,background:col.accent}}/>
                <div style={{flex:1,minWidth:0,paddingLeft:8}}>
                  <div style={{fontWeight:800,fontSize:14,color:BRAND.navy,marginBottom:4,fontFamily:"'Lora',serif"}}>{note.title}</div>
                  {c&&<CPill course={c}/>}
                </div>
                <div style={{display:"flex",gap:6}}>
              ...
              <div style={{padding:"14px 16px",minHeight:48}}>
                {note.content?<div style={{fontSize:13,color:BRAND.slate,lineHeight:1.75}} dangerouslySetInnerHTML={{__html:note.content}}/>:<div style={{fontSize:12,color:BRAND.muted,fontStyle:"italic"}}>No content yet — tap Edit to write.</div>}
  ...
  return(
    <div className="fi">
      ...
      <div style={{display:"flex",gap:16,marginBottom:14,flexWrap:"wrap"}}>
        {Object.entries(TC).map(([t,c])=><div key={t} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:BRAND.slate,fontWeight:500}}><div style={{width:10,height:10,borderRadius:"50%",background:c}}/>{t.charAt(0).toUpperCase()+t.slice(1)}</div>)}
      </div>
      <div style={{background:BRAND.white,borderRadius:18,border:`1px solid ${BRAND.line}`,overflow:"hidden",marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",background:BRAND.navyLt,borderBottom:`1px solid ${BRAND.line}`}}>
          {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><div key={d} style={{textAlign:"center",padding:"8px",fontSize:11,fontWeight:800,color:BRAND.navy,letterSpacing:".05em"}}>{d}</div>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)"}}>
          ...
          {Array(dim).fill(null).map((_,i)=>{
            ...
            const isSel=sel===day;
            return(
              <div key={day} onClick={()=>setSel(isSel?null:day)} style={{minHeight:88,padding:"6px 7px",borderRight:`1px solid ${BRAND.bg}`,borderBottom:`1px solid ${BRAND.bg}`,background:isSel?BRAND.navyLt:isToday?"#F0F7FF":BRAND.white,cursor:"pointer",transition:"background .15s"}}>
                ...
                {evts.slice(0,3).map(ev=><div key={ev.id} style={{fontSize:10,fontWeight:600,padding:"2px 5px",borderRadius:5,background:TC[ev._t]+"20",color:TC[ev._t],marginBottom:2,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",border:`1px solid ${TC[ev._t]}40`}}>{ev.title}</div>)}
        ...
        <table style={{borderCollapse:"collapse",width:"100%",minWidth:64+120*7}}>
          <thead>
            <tr>
              ...
              {TT_DAYS.map(day=><th key={day} style={{width:120,background:BRAND.navy,color:"#fff",fontSize:11,fontWeight:700,padding:"14px 8px",textAlign:"center",borderRight:"1px solid rgba(255,255,255,.10)"}}>{day}</th>)}
            </tr>
          </thead>
          <tbody>
            {TT_SLOTS.map(time=>{
              ...
              return(
                <tr key={time}>
                  ...
                  {TT_DAYS.map(day=>{
                    ...
                    return(
                      <td key={day} onClick={()=>openCell(day,time)} style={{width:120,height:44,padding:"3px 5px",borderRight:`1px solid ${BRAND.bg}`,borderBottom:`1px solid ${BRAND.bg}`,verticalAlign:"top",cursor:"pointer",transition:"background .12s"}}
  ...
  return(
    <div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:24}}>
        <div style={{background:BRAND.white,borderRadius:18,border:`1px solid ${BRAND.line}`,padding:"32px 24px",textAlign:"center"}}>
          <div style={{width:84,height:84,borderRadius:"50%",background:`linear-gradient(135deg,${BRAND.navy},${BRAND.navyMid})`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:30,margin:"0 auto 16px",fontFamily:"'Lora',serif",boxShadow:"0 6px 20px rgba(15,45,82,.30)"}}>{initials}</div>
          <div style={{fontWeight:800,fontSize:17,color:BRAND.navy,fontFamily:"'Lora',serif"}}>{form.first_name} {form.last_name}</div>
...
function Sidebar({ active, onNav, courses, onSelect, collapsed, setCollapsed }) {
  ...
  return(
    <div style={{width:collapsed?64:248,minWidth:collapsed?64:248,height:"100vh",background:BRAND.white,borderRight:`1px solid ${BRAND.line}`,display:"flex",flexDirection:"column",position:"sticky",top:0,transition:"width .25s,min-width .25s",overflow:"hidden",zIndex:10}}>
      <div style={{padding:collapsed?"18px 0":"22px 20px 18px",borderBottom:`1px solid ${BRAND.line}`,display:"flex",alignItems:"center",justifyContent:collapsed?"center":"flex-start"}}>
        {collapsed
          ?<div style={{width:34,height:34,borderRadius:10,background:BRAND.navy,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontWeight:800,fontSize:15,fontFamily:"'Lora',serif"}}>S</span></div>
          :<div><div style={{fontSize:19,fontWeight:800,color:BRAND.navy,fontFamily:"'Lora',serif",letterSpacing:"-.4px"}}>StudyDesk</div><div style={{fontSize:10,color:BRAND.muted,fontWeight:600,marginTop:2,letterSpacing:".04em"}}>UCC · 2025/2026</div></div>
        }
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"14px 8px"}}>
        {!collapsed&&<div style={{fontSize:10,fontWeight:700,color:"#B0BFCF",textTransform:"uppercase",letterSpacing:".08em",padding:"6px 10px 8px"}}>Menu</div>}
        {NAV.map(({k,i,l})=>{
          const isA=active===k||(k==="courses"&&active==="course-detail");
          return(
            <button key={k} onClick={()=>onNav(k)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:collapsed?"11px 0":"10px 12px",justifyContent:collapsed?"center":"flex-start",borderRadius:10,border:"none",cursor:"pointer",background:isA?BRAND.navyLt:"transparent",color:isA?BRAND.navy:BRAND.muted,fontWeight:isA?700:500,fontSize:13,marginBottom:3,fontFamily:"inherit",transition:"all .15s"}}>
        ...
        {!collapsed&&courses.length>0&&(
          <>
            <div style={{fontSize:10,fontWeight:700,color:"#B0BFCF",textTransform:"uppercase",letterSpacing:".08em",padding:"16px 10px 8px"}}>My Courses</div>
            {courses.map(c=>{const col=PAL[c.color_tag]||PAL.Blue;return(
              <button key={c.id} onClick={()=>onSelect(c)} style={{display:"flex",alignItems:"center",gap:9,width:"100%",padding:"7px 10px",borderRadius:9,border:"none",cursor:"pointer",background:"transparent",color:BRAND.muted,fontSize:12,fontWeight:500,marginBottom:2,fontFamily:"inherit",textAlign:"left"}}
      ...
      <div style={{padding:"10px 8px",borderTop:`1px solid ${BRAND.line}`}}>
        <button onClick={()=>setCollapsed(p=>!p)} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",justifyContent:collapsed?"center":"flex-start",borderRadius:9,border:"none",cursor:"pointer",background:"transparent",color:BRAND.muted,fontSize:12,fontFamily:"inherit"}}>
