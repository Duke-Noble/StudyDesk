...
/* ─────────────────────────────────────────────────────────────────────────────
   GLOBAL CSS  — premium deep-navy / warm-gold / slate palette
───────────────────────────────────────────────────────────────────────────── */
const G = `
...
/* ─────────────────────────────────────────────────────────────────────────────
   BRAND COLOURS  — deep navy primary, warm gold accent
───────────────────────────────────────────────────────────────────────────── */
const BRAND = {
  navy:    "#0F2D52",
  navyMid: "#1A4173",
...
/* Course colour tags — richer, more distinguished */
const PAL = {
...
/* Timetable 06:30 → 21:30 in 30-min slots */
...
const IC = {
  ...
  grad:"M12 14l9-5-9-5-9 5 9 5zm0 7v-7m0 0l-9-5m9 5l9-5",
};
...
function Pill({children,color,bg,border,style={}}){
  return(
    <span style={{display:"inline-flex",alignItems:"center",padding:"2px 10px",borderRadius:99,fontSize:11,fontWeight:700,
      color:color||BRAND.slate,background:bg||BRAND.navyLt,border:border?`1px solid ${border}`:undefined,...style}}>
      {children}
    </span>
  );
}
...
function Btn({children,variant="primary",size="md",onClick,disabled,style={},type="button",loading}){
  ...
  const V={
    ...
    gold:     {background:BRAND.goldLt,color:BRAND.gold,border:`1px solid #F0D085`},
  };
  return(
...
function Inp({label,type="text",value,onChange,placeholder,required,min,max,style={}}){
  ...
  return(
    <div style={{marginBottom:14}}>
      {label&&<label style={{fontSize:11,fontWeight:700,color:BRAND.slate,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:".06em"}}>
      ...
      <input type={type} value={value??""} onChange={onChange} placeholder={placeholder} min={min} max={max}
        ...
        style={{width:"100%",padding:"10px 13px",borderRadius:9,
          ...
          fontSize:13,color:BRAND.navy,background:BRAND.white,outline:"none",
...
function ErrBox({msg}){
  return msg?(
    <div style={{padding:"10px 14px",borderRadius:9,background:BRAND.dangerBg,
      border:`1px solid #F5C0BC`,color:BRAND.danger,fontSize:13,marginBottom:14}}>
      {msg}
    </div>
  ):null;
}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(8,18,32,.50)",display:"flex",
      ...
      <div className="fi" style={{background:BRAND.white,borderRadius:18,
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
          padding:"22px 26px 0",borderBottom:`1px solid ${BRAND.line}`,paddingBottom:16}}>
  ...
  return(
    <div style={{textAlign:"center",padding:"56px 24px",color:BRAND.muted}}>
      <div style={{width:64,height:64,borderRadius:18,background:BRAND.navyLt,
        display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",color:BRAND.navy}}>
        <Ic n={icon} size={28}/>
      </div>
      ...
      <div style={{display:"flex",gap:2,padding:"6px 8px",background:BRAND.bg,
        ...
        {tools.map(t=>(
          <button key={t.l} onMouseDown={e=>{e.preventDefault();t.c();}}
            style={{...t.s,minWidth:28,height:28,border:`1px solid ${BRAND.line}`,borderRadius:6,
              cursor:"pointer",background:BRAND.white,color:BRAND.navy,padding:"0 4px"}}>
            {t.l}
          </button>
        ))}
      </div>
...
function Dashboard({courses,deadlines,quizzes,assessments,assignments,notes,user,onNav}){
  ...
  // Greeting — uses profile data, never hardcoded
  ...
  return(
    <div className="fi">
      {/* Header */}
      <div style={{marginBottom:28,padding:"24px 28px",background:BRAND.navy,borderRadius:20,
        color:"#fff",backgroundImage:"linear-gradient(135deg,#0F2D52 0%,#1A4173 60%,#0E3A6E 100%)"}}>
        ...
        {(studentId||programme)&&(
          <p style={{fontSize:12,color:"rgba(255,255,255,.70)",marginTop:8,fontWeight:500}}>
            {studentId}{studentId&&programme?" · ":""}{programme}{programme?" · ":""}{programme?"2025/2026 Semester 2":""}
          </p>
        )}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:20}}>
        ...
        <div style={{background:BRAND.white,borderRadius:16,border:`1px solid ${BRAND.line}`,padding:"20px"}}>
          ...
          {upcoming.length===0
            ?<Empty icon="cal" title="Nothing due this week" sub="You're all caught up — great work!"/>
            :upcoming.map(item=>{
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          {/* Today */}
          <div style={{background:"linear-gradient(135deg,#0F2D52,#1A6DBF)",borderRadius:16,
...
function CoursesPage({courses,reload,onSelect}){
  ...
  const save=async()=>{
    ...
    try{
      const p={course_code:form.course_code,course_name:form.course_name,
        instructor_name:form.instructor_name||null,instructor_email:form.instructor_email||null,
        room:form.room||null,schedule:form.schedule||null,color_tag:form.color_tag||"Blue"};
  ...
  const del=async c=>{
    if(!window.confirm(`Delete "${c.course_code}"? All related notes, deadlines, quizzes, assessments and assignments will also be deleted.`))return;
    try{
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:5}}>
                      ...
                      {!c.instructor_name&&!c.schedule&&!c.room&&(
                        <div style={{fontSize:12,color:"#A0AEC0",fontStyle:"italic"}}>
                          Tap Edit to add course details
                        </div>
                      )}
                    </div>
                  </div>
  ...
  return(
    <div className="fi">
      <button onClick={onBack} style={{display:"flex",alignItems:"center",gap:6,background:"none",
        border:"none",cursor:"pointer",color:BRAND.muted,fontSize:13,fontWeight:600,marginBottom:18,padding:0}}>
      ...
      {tab==="deadlines"&&(
        <div>
          ...
          {cD.length===0?<Empty icon="clock" title="No deadlines" sub="Track your submission deadlines" action="Add Deadline" onAction={()=>open("deadline",null)}/>:
      ...
      {tab==="quizzes"&&(
        <div>
          ...
          {cQ.length===0?<Empty icon="doc" title="No quizzes" sub="Track upcoming quizzes" action="Add Quiz" onAction={()=>open("quiz",null)}/>:
      ...
      {tab==="assignments"&&(
        <div>
          ...
          {cX.length===0?<Empty icon="doc" title="No assignments" sub="Track your assignments" action="Add Assignment" onAction={()=>open("assignment",null)}/>:
...
function CalendarPage({courses,deadlines,quizzes,assessments,assignments,reloadAll}){
  ...
  const delEvt=async ev=>{if(!window.confirm("Delete this event?"))return;try{await db.del(TBL[ev._t],ev.id);await reloadAll();}catch(e){window.alert(e.message);}};
    ...
    <div className="fi">
      ...
      <div style={{background:BRAND.white,borderRadius:18,border:`1px solid ${BRAND.line}`,overflow:"hidden",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
          padding:"18px 22px",borderBottom:`1px solid ${BRAND.line}`,
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",background:BRAND.navyLt,borderBottom:`1px solid ${BRAND.line}`}}>
          {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=>(
            <div key={d} style={{textAlign:"center",padding:"9px",fontSize:11,fontWeight:800,color:BRAND.navy,letterSpacing:".05em"}}>{d}</div>
          ))}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)"}}>
          {Array(first).fill(null).map((_,i)=><div key={`e${i}`} style={{minHeight:90,borderRight:`1px solid ${BRAND.bg}`,borderBottom:`1px solid ${BRAND.bg}`}}/>)}
          {Array(dim).fill(null).map((_,i)=>{
            ...
            return(
              <div key={day} onClick={()=>setSel(isSel?null:day)}
                style={{minHeight:90,padding:"6px 8px",borderRight:`1px solid ${BRAND.bg}`,
                  borderBottom:`1px solid ${BRAND.bg}`,
                  background:isSel?BRAND.navyLt:isToday?"#F0F7FF":BRAND.white,
                  cursor:"pointer",transition:"background .15s"}}>
                <div style={{fontSize:13,fontWeight:isToday?800:500,width:26,height:26,borderRadius:"50%",
                  display:"flex",alignItems:"center",justifyContent:"center",
                  background:isToday?BRAND.navy:"transparent",
                  color:isToday?"#fff":BRAND.slate,marginBottom:4}}>
                  {day}
                </div>
                {evts.slice(0,3).map(ev=>(
                  <div key={ev.id} style={{fontSize:10,fontWeight:600,padding:"2px 5px",borderRadius:5,
                    background:TC[ev._t]+"20",color:TC[ev._t],marginBottom:2,
                    overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",
                    border:`1px solid ${TC[ev._t]}40`}}>
                    {ev.title}
                  </div>
                ))}
                {evts.length>3&&<div style={{fontSize:10,color:BRAND.muted}}>+{evts.length-3} more</div>}
              </div>
            );
          })}
        </div>
        ...
        <div style={{background:BRAND.white,borderRadius:16,border:`1px solid ${BRAND.line}`,padding:"20px"}}>
          ...
          {selEvts.length===0?<Empty icon="cal" title="No events" sub="Tap '+ Add' to schedule something"/>:
            selEvts.map(ev=>{const c=gc(ev.course_id);const col=c?PAL[c.color_tag]:PAL.Blue;return(
              <div key={ev.id} style={{display:"flex",gap:10,padding:"12px 15px",borderRadius:11,
                marginBottom:9,background:col.bg,border:`1px solid ${col.border}`,alignItems:"center"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:BRAND.muted,display:"flex",gap:8,marginTop:3}}>
                    {c&&<CPill course={c}/>}
                    <span style={{color:TC[ev._t],fontWeight:600}}>{ev._t}</span>
                    <span>· {fmtTime(ev._d)}</span>
                  </div>
                </div>
                <div style={{display:"flex",gap:6}}>
    ...
    <div className="fi">
      ...
      <div style={{background:BRAND.white,borderRadius:18,border:`1px solid ${BRAND.line}`,overflow:"auto"}}>
        <table style={{borderCollapse:"collapse",width:"100%",minWidth:64+120*7}}>
          <thead>
            <tr>
              <th style={{width:64,background:BRAND.navy,color:"#fff",fontSize:11,fontWeight:700,
                padding:"14px 10px",textAlign:"center",borderRight:"1px solid rgba(255,255,255,.12)",
                position:"sticky",left:0,zIndex:2,letterSpacing:".04em"}}>Time</th>
              {TT_DAYS.map(day=>(
                <th key={day} style={{width:120,background:BRAND.navy,color:"#fff",fontSize:11,fontWeight:700,
                  padding:"14px 8px",textAlign:"center",borderRight:"1px solid rgba(255,255,255,.10)",
                  letterSpacing:".03em"}}>
                  {day}
                </th>
              ))}
          ...
          <tbody>
            {TT_SLOTS.map(time=>{
              ...
              return(
                <tr key={time} style={{background:isHour?BRAND.navyLt:BRAND.white}}>
                  <td style={{width:64,fontSize:isHour?12:10,fontWeight:isHour?700:400,
                    color:isHour?BRAND.navy:BRAND.muted,padding:"0 10px",textAlign:"right",
                    borderRight:`1px solid ${BRAND.line}`,borderBottom:`1px solid ${BRAND.bg}`,
                    height:44,verticalAlign:"middle",
                    background:isHour?BRAND.navyLt:BRAND.white,
                  ...
                  {TT_DAYS.map(day=>{
                    ...
                    return(
                      <td key={day} onClick={()=>openCell(day,time)}
                        style={{width:120,height:44,padding:"3px 5px",
                          borderRight:`1px solid ${BRAND.bg}`,borderBottom:`1px solid ${BRAND.bg}`,
                          verticalAlign:"top",cursor:"pointer",transition:"background .12s"}}
                        ...
                        {slot&&(
                          <div style={{background:col.soft,border:`1px solid ${col.border}`,
                            borderRadius:7,padding:"3px 7px",height:"100%",
                            boxSizing:"border-box",overflow:"hidden"}}>
                            <div style={{fontSize:11,fontWeight:800,color:col.text,
                              overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>
                              {course?course.course_code:slot.label||"Class"}
                            </div>
                            {(slot.label&&course)&&(
                              <div style={{fontSize:9,color:col.text,opacity:.7,
                                overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>
                                {slot.label}
                              </div>
                            )}
      ...
      {modal&&(
        <Modal title={`${modal.mode==="add"?"Add Slot":"Edit Slot"} — ${modal.day} ${modal.time}`} onClose={()=>setModal(null)}>
          ...
          <Inp label="Label (optional)" value={form.label||""} onChange={f("label")} placeholder="e.g. Lab session, Tutorial…"/>
          <ColorPicker value={form.color_tag||"Blue"} onChange={v=>setForm(p=>({...p,color_tag:v}))}/>
          <div style={{display:"flex",gap:10,justifyContent:"space-between",marginTop:8}}>
            <div>{modal.mode==="edit"&&<Btn variant="danger" onClick={del}>Remove Slot</Btn>}</div>
...
/* ─────────────────────────────────────────────────────────────────────────────
   PROFILE PAGE  — programme is editable, never hardcoded
───────────────────────────────────────────────────────────────────────────── */
function ProfilePage({session,onSignOut}){
  ...
  const save=async()=>{
    ...
    const{error}=await supabase.auth.updateUser({data:{
      ...
      student_id:form.student_id,programme:form.programme,level:form.level,
    }});
    setSaving(false);
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:24}}>
        ...
        <div style={{background:BRAND.white,borderRadius:18,border:`1px solid ${BRAND.line}`,
          ...
          {form.level&&(
            <div style={{marginTop:10}}>
              <Pill color={BRAND.navy} bg={BRAND.navyLt}>Level {form.level}</Pill>
            </div>
          )}
        ...
        <div style={{background:BRAND.white,borderRadius:18,border:`1px solid ${BRAND.line}`,padding:"28px"}}>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:12,alignItems:"center",marginTop:6}}>
            {msg&&(
              <span style={{fontSize:13,color:msg.startsWith("✓")?BRAND.success:BRAND.danger,fontWeight:600}}>
                {msg}
              </span>
            )}
  ...
  return(
    <div style={{width:collapsed?64:248,minWidth:collapsed?64:248,height:"100vh",
      ...
      <div style={{padding:collapsed?"18px 0":"22px 20px 18px",
        borderBottom:`1px solid ${BRAND.line}`,display:"flex",alignItems:"center",
        ...
        {collapsed
          ...
          :<div>
              ...
              <div style={{fontSize:10,color:BRAND.muted,fontWeight:600,marginTop:2,
                letterSpacing:".04em"}}>UCC · 2025/2026</div>
            </div>
        }
      </div>
...
export default function StudyDesk({session,onSignOut}){
  ...
  const renderMain=()=>{
    ...
    switch(view){
      case "dashboard":    return <Dashboard courses={courses} deadlines={deadlines} quizzes={quizzes} assessments={assessments} assignments={assignments} notes={notes} user={session?.user} onNav={nav}/>;
      case "courses":      return <CoursesPage courses={courses} reload={rC} onSelect={selectCourse}/>;
      case "course-detail":
      ...
      case "notes":        return <NotesPage notes={notes} courses={courses} reload={rN}/>;
      case "calendar":     return <CalendarPage courses={courses} deadlines={deadlines} quizzes={quizzes} assessments={assessments} assignments={assignments} reloadAll={reloadAll}/>;
      case "timetable":    return <TimetablePage courses={courses} slots={slots} reloadSlots={rS}/>;
      case "profile":      return <ProfilePage session={session} onSignOut={onSignOut}/>;
      default: return null;
    }
  };
